...
for (i in 1:nrow(uk_grid)){
  i <- runif(1,min = 0, max = 2000) %>% round() #for testing by using random grid squares
  ...
  leaflet() %>%
    addTiles() %>%
    addPolygons(data = wet_features_data$osm_polygons) %>%
    addPolygons(data = buffered_water_points,label = rownames(buffered_water_points)) %>%
