...
for (i in 1:nrow(uk_grid)){
  ...
  distance_check_bad <- rep(0,nrow(raster_this_grid))
  distance_check_good <- rep(0,nrow(raster_this_grid))
  ...
  if (!is.null(good_features_data$osm_lines)) {
    distance_check_good <- distance_check_good + rowSums(st_is_within_distance(raster_as_sf,good_features_data$osm_lines,dist = 175,sparse = FALSE))
  }
  ...
  if (length(offline_good_features_data)>0){
    for (ii in 1:length(offline_good_features_data)){
      ...
      tryCatch(
        {
        distance_check_good <- distance_check_good + rowSums(st_is_within_distance(raster_as_sf,offline_good_features_data[[ii]]$geometry,dist = 175,sparse = FALSE))
  ...
  # do the same for the OSM bad features
  #polygons
  ...
  raster_as_sf$composite[raster_as_sf$access>0] <- 1
  raster_as_sf$composite[raster_as_sf$no_go>0] <- 0
  raster_as_sf$composite[raster_as_sf$no_go>0]
